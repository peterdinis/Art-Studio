...
describe('useArtStudioStore', () => {
    beforeEach(() => {
        ...
        const { setActiveTool, setPrimaryColor, clearHistory } = useArtStudioStore.getState();
        setActiveTool('brush');
    ...
    it('should swap primary and secondary colors', () => {
        const { setPrimaryColor, setSecondaryColor, swapColors } = useArtStudioStore.getState();
    ...
    it('should handle history (add/undo/redo)', () => {
        const { addToHistory, undo, redo, canUndo, canRedo } = useArtStudioStore.getState();
