...
export type Tool = 
  | 'brush' | 'pencil' | 'eraser' | 'fill' | 'gradient' | 'eyedropper' 
  | 'clone' | 'healing' | 'blur'
  | 'rectangle' | 'ellipse' | 'polygon' | 'line' | 'pen' | 'text'
  | 'select' | 'marquee' | 'lasso' | 'magicwand'
  | 'move' | 'hand' | 'zoom';
...
interface ArtStudioState {
  ...
  historyIndex: number;
  addToHistory: (canvasData: string, thumbnail?: string, action?: string) => void;
...
export const useArtStudioStore = create<ArtStudioState>((set, get) => ({
  // Tools
  activeTool: 'brush' as unknown as Tool,
  ...
  recentColors: ['#ffffff', '#000000', '#ff0000', '#00ff00', '#0000ff', '#ffff00', '#ff00ff', '#00ffff'],
  ...
  canvasSize: null,
  setCanvasSize: (size) => set({ canvasSize: size, history: [], historyIndex: -1 }),
  ...
  clearHistory: () => {
    ...
    if (history.length > 0) {
      ...
      set({ history: currentEntry ? [currentEntry] : [], historyIndex: currentEntry ? 0 : -1 });
    }
  },
