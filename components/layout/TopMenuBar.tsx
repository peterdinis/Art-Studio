...
export const TopMenuBar: React.FC = () => {
  ...
  const handleApplyFilter = (filter: string) => {
    ...
    switch (filter) {
      case 'Invert':
        if (activeObject) {
          ...
          if (typeof currentFill === 'string' && currentFill.startsWith('#')) {
            const inverted = '#' + (0xFFFFFF ^ parseInt(currentFill.slice(1), 16)).toString(16).padStart(6, '0');
      ...
      case 'Brightness/Contrast':
        const brightnessValue = prompt('Enter brightness adjustment (-100 to 100):', '0');
        if (brightnessValue) {
          ...
          if (!isNaN(value)) {
            ...
            canvas.getObjects().forEach((obj: any) => {
              ...
              const newOpacity = Math.max(0.1, Math.min(1, currentOpacity + value / 200));
  ...
  const editMenu: MenuItemConfig[] = [
    ...
    { 
      ...
      submenu: [
        ...
        { label: 'Content-Aware Fill', icon: Wand2, action: () => toast.info('Content-aware fill applied') },
      ]
    },
  ];
  ...
  const viewMenu: MenuItemConfig[] = [
    ...
    { 
      ...
      action: () => {
        ...
        toast.success(showRightPanel ? 'Right panel hidden' : 'Right panel shown');
      },
  ...
  const imageMenu: MenuItemConfig[] = [
    ...
    { separator: true, label: '' },
    { label: 'Rotate Canvas 90° CW', icon: RotateCw, action: () => handleRotateCanvas(90) },
    { label: 'Rotate Canvas 90° CCW', icon: RotateCcw, action: () => handleRotateCanvas(-90) },
    { label: 'Flip Canvas Horizontal', icon: FlipHorizontal, action: () => handleFlipCanvas('horizontal') },
    { label: 'Flip Canvas Vertical', icon: FlipVertical, action: () => handleFlipCanvas('vertical') },
  ];
  ...
  const layerMenu: MenuItemConfig[] = [
    ...
    { separator: true, label: '' },
    { label: 'Merge Down', icon: Merge, shortcut: '⌘E', action: handleMergeLayers },
    { label: 'Merge Visible', icon: Merge, shortcut: '⇧⌘E', action: handleMergeLayers },
  ...
  const windowMenu: MenuItemConfig[] = [
    { 
      ...
      action: () => {
        ...
        toast.success(showBrushesPanel ? 'Brushes panel hidden' : 'Brushes panel shown');
      },
    ...
    { 
      ...
      action: () => {
        ...
        toast.success(showColorsPanel ? 'Colors panel hidden' : 'Colors panel shown');
      },
    ...
    { 
      ...
      action: () => {
        ...
        toast.success(showLayersPanel ? 'Layers panel hidden' : 'Layers panel shown');
      },
    ...
    { 
      ...
      action: () => {
        ...
        toast.success(showHistoryPanel ? 'History panel hidden' : 'History panel shown');
      },
  ...
  const helpMenu: MenuItemConfig[] = [
    ...
    { separator: true, label: '' },
    { label: 'Send Feedback', icon: MessageCircle, action: () => {
      const feedback = prompt('Enter your feedback:');
      if (feedback) {
        toast.success('Thank you for your feedback!');
      }
    }},
    { label: 'About ArtStudio', icon: Info, action: () => {
      toast.info('ArtStudio v1.0 - Professional Digital Art Studio', { duration: 5000 });
    }},
  ];
  ...
  return (
    <div className="h-10 bg-card border-b border-border flex items-center justify-between px-2 animate-fade-in">
      ...
      <div className="flex items-center gap-1">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button onClick={() => window.open('https://docs.lovable.dev', '_blank')} className="tool-button w-8 h-8">
