...
export const TextOptionsPanel: React.FC = () => {
	...
	useEffect(() => {
		...
		if (activeText) {
			...
			setSelectedTextIndex(
				textObjects.findIndex((t) => t.id === activeText.id),
			);
	...
	const navigateText = useCallback(
		(direction: "prev" | "next") => {
	...
	const createNewText = useCallback(() => {
		...
		const newText = {
			...
			textAlign: safeBrushSettings.textAlign as
				| "left"
				| "center"
				| "right"
				| "justify",
	...
	const applySettingsToAllTexts = useCallback(() => {
		...
		if (
			confirm(`Aplikovať nastavenia na všetkých ${textObjects.length} textov?`)
		) {
					...
					<div className="space-y-1 max-h-32 overflow-y-auto">
						{textObjects.map((text, index) => (
							...
							>
								...
								<div className="flex gap-1">
									{text.isEditing && (
										<span className="text-[10px] text-green-600 px-1 py-0.5 bg-green-500/10 rounded">
											EDIT
										</span>
									)}
							...
							style={{
								...
								wordWrap:
									safeBrushSettings.textWrap === "none"
										...
										: "break-word",
								whiteSpace:
									safeBrushSettings.textWrap === "none" ? "nowrap" : "normal",
				...
				<div className="pt-3 border-t">
					...
					<div className="text-xs text-muted-foreground grid grid-cols-2 gap-2">
						<div className="space-y-1">
							...
							<div>
								Aktívne editácie:{" "}
								{textObjects.filter((t) => t.isEditing).length}
							</div>
							<div>
								Celkové znaky:{" "}
								{textObjects.reduce((acc, t) => acc + t.text.length, 0)}
							</div>
						</div>
						<div className="space-y-1">
							...
							<div>
								Pozícia:{" "}
								{activeText
									? `${Math.round(activeText.x)}, ${Math.round(activeText.y)}`
									: "N/A"}
							</div>
							<div>
								Aktuálny: {selectedTextIndex + 1}/{textObjects.length}
							</div>
						</div>
					</div>
				</div>
