...
export const GradientOptionsPanel: React.FC = () => {
	...
	const handleColorStopChange = (index: number, updates: Partial<{ color: string; position: number }>) => {
		const newStops = [...brushSettings.gradientStops];
	...
	const handleReverseGradient = () => {
		const reversedStops = brushSettings.gradientStops.map(stop => ({
			...stop,
			...
			<div className="p-4 space-y-6">
				...
				<div className="space-y-3">
					...
					<div 
						...
						style={{
							background: brushSettings.gradientType === "linear" 
								...
								<div className="space-y-2">
									...
									<div>
										...
										<Slider
											...
											step={0.01}
											value={[stop.position]}
											onValueChange={([value]) => handleColorStopChange(index, { position: value })}
											className="mt-2"
										/>
									</div>
								</div>
				...
				<div className="space-y-3">
					...
					<div className="grid grid-cols-3 gap-2">
						{[
							...
							{ name: "Ocean", stops: [{ color: "#4ECDC4", position: 0 }, { color: "#556270", position: 1 }] },
							{ name: "Forest", stops: [{ color: "#2AF598", position: 0 }, { color: "#0093E9", position: 1 }] },
							{ name: "Purple", stops: [{ color: "#8A2387", position: 0 }, { color: "#F27121", position: 1 }] },
							{ name: "Steel", stops: [{ color: "#757F9A", position: 0 }, { color: "#D7DDE8", position: 1 }] },
							{ name: "Rainbow", stops: [
							...
							<Button
								...
								onClick={() => setBrushSettings({ gradientStops: preset.stops })}
							>
								<div 
