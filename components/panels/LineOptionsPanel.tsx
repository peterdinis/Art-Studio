...
export const LineOptionsPanel: React.FC<LineOptionsPanelProps> = ({
	className,
}) => {
	const { brushSettings, setBrushSettings, primaryColor, activeTool } =
		useArtStudioStore();
	...
	const [lineOptions, setLineOptions] = useState<LineOptionsState>({
		...
		lineStyle:
			(brushSettings.lineSettings?.lineStyle as LineStyle) || "straight",
	...
	return (
		<div className={`space-y-4 p-4 ${className}`}>
				...
				{lineOptions.arrowType !== "none" && (
					<div className="space-y-2 pt-2">
						...
						<Slider
							...
							onValueChange={(value) =>
								applyLineOptions({ arrowSize: value[0] })
							}
							min={5}
							max={30}
							step={1}
			...
			{(lineOptions.lineType === "dashed" ||
				lineOptions.lineType === "dotted" ||
				lineOptions.lineType === "dash-dot") && (
				<div className="space-y-2">
			...
			<div className="grid grid-cols-2 gap-3">
				<div className="space-y-2">
					...
					<Select
						...
						onValueChange={(value: CapType) =>
							applyLineOptions({ capType: value })
						}
					>
						<SelectTrigger className="w-full h-8 text-xs">
							<SelectValue />
				...
				<div className="space-y-2">
					...
					<Select
						...
						onValueChange={(value: JoinType) =>
							applyLineOptions({ joinType: value })
						}
					>
						<SelectTrigger className="w-full h-8 text-xs">
							<SelectValue />
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									applyLineOptions({ isPerfect: checked })
								}
							/>
						</div>
						...
						{lineOptions.lineStyle === "curved" && (
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<span className="text-xs text-muted-foreground">
										{lineOptions.tension.toFixed(1)}
									</span>
								</div>
								<Slider
									value={[lineOptions.tension]}
									onValueChange={(value) =>
										applyLineOptions({ tension: value[0] })
									}
									min={0}
									max={1}
									step={0.1}
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								...
								<span className="text-xs text-muted-foreground">
									{lineOptions.precision}px
								</span>
							</div>
							<Slider
								value={[lineOptions.precision]}
								onValueChange={(value) =>
									applyLineOptions({ precision: value[0] })
								}
								min={0}
								max={50}
								step={1}
			...
			<div className="pt-2">
				...
				<div className="h-20 border border-dashed border-border rounded-md flex items-center justify-center bg-muted/20">
					<div className="relative w-32">
						...
						{/* Arrow preview */}
						{lineOptions.arrowType === "start" ||
						lineOptions.arrowType === "both" ? (
							<div
