...
const KonvaCanvas: React.FC<KonvaCanvasProps> = ({
  ...
  const applyDodgeBrush = useCallback((x: number, y: number) => {
    ...
    if (!ctx) return;
    const size = brushSettings.size;
    const intensity = (brushSettings.dodgeIntensity || 50) / 200;
    const imgData = ctx.getImageData(x - size / 2, y - size / 2, size, size);
    const d = imgData.data;
    for (let i = 0; i < d.length; i += 4) {
  ...
  }, [activeTool, selectionBounds, actualBackground, lines, shapes, images, gradients, setCanvasSize, saveCanvasState]);
  ...
  }, [primaryColor, activeLayerId, brushSettings.fontSize, setShapes, saveCanvasState]);
  ...
  }, [isDrawing, activeDrawingLine, tempContext, activeTool, handleCloneBrush, applyHealingBrush, applyBlurBrush, applyDodgeBrush, applyBurnBrush]);
  ...
  const handleMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (activeTool === "star") {
      ...
      const starShape: ShapeObject = {
        ...
        fill: brushSettings.starFillType === "none" ? "transparent" : 
              (brushSettings.starFillType === "gradient" ? "transparent" : 
  ...
  const handleMouseMove = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (currentShape && shapeStartPoint.current) {
      if (currentShape.type === "rect") {
        ...
        const outerRadius = Math.sqrt((pos.x - currentShape.x) ** 2 + (pos.y - currentShape.y) ** 2);
        const innerRadius = outerRadius * (brushSettings.starInnerRadius / brushSettings.starOuterRadius);
        setCurrentShape({ 
          ...currentShape, 
          outerRadius,
          innerRadius
  ...
  const handleMouseUp = () => {
      ...
      if (activeTool === "marquee" && selectionBounds) {
        ...
        const selectedShapes = shapes.filter(s => {
          ...
          if (s.type === "star") {
            ...
            return s.x - starRadius < x + width && s.x + starRadius > x && s.y - starRadius < y + height && s.y + starRadius > y;
          }
    ...
    if (currentShape && ["rect", "ellipse", "polygon", "line", "star"].includes(currentShape.type)) {
      setShapes((prev) => [...prev, currentShape]);
      setCurrentShape(null);
    ...
    if (["clone", "healing", "blur", "dodge", "burn"].includes(activeTool)) {
      if (tempContext) {
        ...
        >
          <Layer ref={layerRef}>
            ...
            {shapes.filter(s => isLayerVisible(s.layerId)).map(shape => {
              ...
              switch (shape.type) {
                case "rect":
                  return <Rect key={shape.id} id={shape.id} x={shape.x} y={shape.y} width={shape.width} height={shape.height} fill={shape.fill} stroke={shape.stroke} strokeWidth={shape.strokeWidth} rotation={shape.rotation} scaleX={shape.scaleX} scaleY={shape.scaleY} {...commonProps} />;
                case "ellipse":
                  return <Ellipse key={shape.id} id={shape.id} x={shape.x} y={shape.y} radiusX={shape.radiusX || 0} radiusY={shape.radiusY || 0} fill={shape.fill} stroke={shape.stroke} strokeWidth={shape.strokeWidth} rotation={shape.rotation} scaleX={shape.scaleX} scaleY={shape.scaleY} {...commonProps} />;
                case "polygon":
                  return <Line key={shape.id} id={shape.id} points={shape.points} closed fill={shape.fill} stroke={shape.stroke} strokeWidth={shape.strokeWidth} rotation={shape.rotation} scaleX={shape.scaleX} scaleY={shape.scaleY} lineJoin="round" lineCap="round" {...commonProps} />;
                case "text":
                  return <Text key={shape.id} id={shape.id} text={shape.text} x={shape.x} y={shape.y} fontSize={shape.fontSize} fill={shape.fill} rotation={shape.rotation} scaleX={shape.scaleX} scaleY={shape.scaleY} {...commonProps} />;
                case "line":
                  return <Line key={shape.id} id={shape.id} points={shape.points} stroke={shape.stroke} strokeWidth={shape.strokeWidth} lineCap="round" lineJoin="round" {...commonProps} />;
                case "star":
                  return <KonvaStar
                    key={shape.id}
                    id={shape.id}
            ...
            {currentShape && currentShape.type === "rect" && 
              <Rect id="preview-rect" x={currentShape.x} y={currentShape.y} width={currentShape.width} height={currentShape.height} fill={currentShape.fill} stroke={currentShape.stroke} strokeWidth={currentShape.strokeWidth} />}
            
            {currentShape && currentShape.type === "ellipse" && 
              <Ellipse id="preview-ellipse" x={currentShape.x} y={currentShape.y} radiusX={currentShape.radiusX || 0} radiusY={currentShape.radiusY || 0} fill={currentShape.fill} stroke={currentShape.stroke} strokeWidth={currentShape.strokeWidth} />}
            
            {currentShape && currentShape.type === "polygon" && 
              <Line id="preview-polygon" points={currentShape.points} closed={true} stroke={currentShape.stroke} strokeWidth={currentShape.strokeWidth} lineJoin="round" lineCap="round" />}
            
            {currentShape && currentShape.type === "line" && 
              <Line id="preview-line" points={currentShape.points} stroke={currentShape.stroke} strokeWidth={currentShape.strokeWidth} />}
